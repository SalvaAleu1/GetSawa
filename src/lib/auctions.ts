...
export async function createVerifiedAuction(params: {
  ...
  await validatePremiumMarketplaceEconomics({
    source: meta.source as "GETSAWA_INVENTORY" | "CUSTOMER_CUSTODY",
    salePriceCents: protectedSalePrice,
    acquisitionCostCents: meta.acquisitionCostCents,
    commissionBps: meta.commissionBps,
  });

  return prisma.$transaction(async (tx) => {
    const activeLink = await tx.$queryRaw<Array<{ auction_id: string }>>`
      SELECT ai."auction_id" FROM "auction_inventory" ai
      JOIN "Auction" a ON a."id"=ai."auction_id"
      WHERE ai."premium_domain_id"=${listing.id}
        AND a."status" IN ('DRAFT','SCHEDULED','LIVE','ENDED','PAID')
        AND ai."released_at" IS NULL
      LIMIT 1
    `;
    ...
    const auction = await tx.auction.create({
      data: {
        title: params.title.trim() || listing.domainName,
        domainName: listing.domainName,
        startingBidCents: params.startingBidCents,
        reservePriceCents: params.reservePriceCents || null,
        minIncrementCents: params.minIncrementCents,
        startAt: params.startAt,
        endAt: params.endAt,
        extensionSeconds: params.extensionSeconds,
        isFeatured: params.isFeatured,
        status: params.startAt <= now ? "LIVE" : "SCHEDULED",
      },
    });
    await tx.$executeRaw`
      INSERT INTO "auction_inventory" ("auction_id","premium_domain_id","source","seller_user_id","commission_bps","acquisition_cost_cents")
      VALUES (${auction.id},${listing.id},${meta.source},${meta.sellerUserId},${meta.commissionBps},${meta.acquisitionCostCents})
    `;
...
export async function getAuctionInventory(auctionId: string) {
  ...
  }>>`SELECT * FROM "auction_inventory" WHERE "auction_id"=${auctionId} LIMIT 1`;
  return rows[0] ?? null;
}
    ...
    try {
      return await prisma.$transaction(async (tx) => {
        const [auction, user] = await Promise.all([
          ...
          tx.user.findUnique({ where: { id: userId }, select: { emailVerifiedAt: true, isSuspended: true } }),
        ]);
        ...
        const inventoryRows = await tx.$queryRaw<Array<{ seller_user_id: string | null; released_at: Date | null; premium_domain_id: string }>>`
          SELECT "seller_user_id","released_at","premium_domain_id" FROM "auction_inventory" WHERE "auction_id"=${auction.id} LIMIT 1
        `;
        ...
        if (auction.endAt.getTime() - now.getTime() <= closingWindowMs) {
          await tx.auction.update({ where: { id: auction.id }, data: { endAt: new Date(now.getTime() + closingWindowMs) } });
        }
        return bid;
...
export async function closeAuctionIfExpired(auctionId: string) {
  return prisma.$transaction(async (tx) => {
    ...
    const inventoryRows = await tx.$queryRaw<Array<{ premium_domain_id: string; released_at: Date | null }>>`
      SELECT "premium_domain_id","released_at" FROM "auction_inventory" WHERE "auction_id"=${auction.id} LIMIT 1
    `;
    const inventory = inventoryRows[0];
    if (!inventory || inventory.released_at) throw new AuctionError("Auction inventory linkage is missing or already released.");

    const highest = await tx.auctionBid.findFirst({ where: { auctionId: auction.id }, orderBy: { amountCents: "desc" } });
    const reserveMet = Boolean(highest) && (!auction.reservePriceCents || highest!.amountCents >= auction.reservePriceCents);
    ...
    await tx.$executeRaw`
      UPDATE "premium_inventory_meta" SET "reserved_by_user_id"=${highest.userId}, "reserved_until"=${paymentDeadline}, "updated_at"=CURRENT_TIMESTAMP
      WHERE "premium_domain_id"=${inventory.premium_domain_id} AND "sold_at" IS NULL
    `;
    await tx.$executeRaw`
      INSERT INTO "auction_orders" ("auction_id","winner_user_id","amount_cents","currency","status","payment_deadline")
      VALUES (${auction.id},${highest.userId},${highest.amountCents},'USD','PAYMENT_PENDING',${paymentDeadline})
      ON CONFLICT ("auction_id") DO UPDATE SET "winner_user_id"=EXCLUDED."winner_user_id", "amount_cents"=EXCLUDED."amount_cents", "payment_deadline"=EXCLUDED."payment_deadline", "updated_at"=CURRENT_TIMESTAMP
    `;
    return updated;
  });
}
...
export async function expireUnpaidAuctionWinners(limit = 200) {
  const rows = await prisma.$queryRaw<Array<{ auction_id: string; premium_domain_id: string; winner_user_id: string }>>`
    SELECT ao."auction_id", ai."premium_domain_id", ao."winner_user_id"
    FROM "auction_orders" ao JOIN "auction_inventory" ai ON ai."auction_id"=ao."auction_id"
    WHERE ao."status"='PAYMENT_PENDING' AND ao."payment_deadline" < CURRENT_TIMESTAMP
    ORDER BY ao."payment_deadline" ASC LIMIT ${limit}
  `;
  let expired = 0;
...
export async function markAuctionPaymentCreated(params: { auctionId: string; winnerUserId: string; orderId: string; orderItemId: string; providerOrderId: string }) {
  const changed = await prisma.$executeRaw`
    UPDATE "auction_orders" SET "order_id"=${params.orderId}, "order_item_id"=${params.orderItemId}, "provider_order_id"=${params.providerOrderId}, "updated_at"=CURRENT_TIMESTAMP
    WHERE "auction_id"=${params.auctionId} AND "winner_user_id"=${params.winnerUserId} AND "status"='PAYMENT_PENDING'
  `;
...
export async function markAuctionPaymentCaptured(params: { auctionId: string; orderId: string; captureId: string }) {
  await prisma.$transaction(async (tx) => {
    const changed = await tx.$executeRaw`
      UPDATE "auction_orders" SET "status"='FULFILLMENT_PENDING', "provider_capture_id"=${params.captureId}, "updated_at"=CURRENT_TIMESTAMP
      WHERE "auction_id"=${params.auctionId} AND "order_id"=${params.orderId} AND "status"='PAYMENT_PENDING'
    `;
