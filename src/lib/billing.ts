...
import { computeTldPrice, generateInvoiceNumber, generateOrderNumber } from "@/lib/pricing";
...
function mapSubscription(row: Record<string, unknown>): BillingSubscriptionRow {
  return {
    ...
    autoRenew: Boolean(row.auto_renew), cancelAtPeriodEnd: Boolean(row.cancel_at_period_end), failedPaymentCount: Number(row.failed_payment_count),
...
export async function ensureDomainSubscription(domainId: string): Promise<void> {
  ...
  const nextBillingAt = new Date(Math.max(now.getTime(), periodEnd.getTime() - 7 * 86400000));
  await prisma.$executeRaw`
    ...
    VALUES
      (${crypto.randomUUID()},${domain.userId},${domain.id},'paypal','ACTIVE','YEARLY',${estimatedPrice},'USD',${periodStart},${periodEnd},${nextBillingAt},${domain.autoRenew})
    ON CONFLICT ("domain_id") DO UPDATE SET
      ...
      "next_billing_at"=CASE WHEN "billing_subscriptions"."status"='ACTIVE' THEN EXCLUDED."next_billing_at" ELSE "billing_subscriptions"."next_billing_at" END,
      "updated_at"=CURRENT_TIMESTAMP
  `;
}

export async function syncDomainSubscriptions(limit = 500): Promise<number> {
  const domains = await prisma.domain.findMany({ where: { status: { in: ["ACTIVE", "EXPIRING"] }, expiresAt: { not: null } }, select: { id: true }, take: limit });
  let synced = 0;
  for (const domain of domains) {
    try { await ensureDomainSubscription(domain.id); synced++; }
    catch (error) {
      await logAudit({ actorId: null, action: "billing.subscription_sync_failed", resource: "domain", resourceId: domain.id, metadata: { reason: error instanceof Error ? error.message.slice(0, 300) : "Unknown error" } });
    }
  }
  return synced;
}
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    try {
        ...
        const created = await tx.order.create({
          data: {
            ...
            invoice: { create: {
              ...
              currency: priced.currency,
              status: "UNPAID",
              billingName: `${domain.user.firstName} ${domain.user.lastName}`,
              billingEmail: domain.user.email,
              billingCountry: domain.user.country,
            } },
          },
        });
        await tx.$executeRaw`
      ...
      await notifyOrderLifecycle({
        ...
        type: "DOMAIN_RENEWAL_INVOICE",
        title: `Renewal payment required for ${domain.name}`,
        body: `Your ${domain.name} renewal invoice is ready. The price was refreshed against the registrar before issue. Complete payment before the domain expires.`,
        emailSubject: `Renewal payment required for ${domain.name}`,
        emailHtml: `<p>Your renewal invoice for <strong>${escapeHtml(domain.name)}</strong> is ready.</p><p>Amount due: <strong>${escapeHtml((priced.totalCents / 100).toFixed(2))} ${escapeHtml(priced.currency)}</strong>.</p><p>Please complete payment before the registrar expiry date.</p>`,
      });
...
export async function processDueRenewals(limit = 100): Promise<RenewalResult> {
  await syncDomainSubscriptions(500);
  const now = new Date();
...
export async function markRenewalPaid(orderId: string): Promise<void> {
  ...
  await prisma.$executeRaw`
    UPDATE "billing_subscriptions" SET
      "status"='ACTIVE',"current_period_start"=${subscription.currentPeriodEnd},"current_period_end"=${newEnd},
      "next_billing_at"=${newNext},"grace_until"=NULL,"failed_payment_count"=0,
      "last_payment_at"=CURRENT_TIMESTAMP,"updated_at"=CURRENT_TIMESTAMP
    WHERE "id"=${subscription.id}
  `;
  await prisma.$executeRaw`
    UPDATE "billing_renewal_attempts" SET "status"='PAID',"attempted_at"=COALESCE("attempted_at",CURRENT_TIMESTAMP),"completed_at"=CURRENT_TIMESTAMP,"updated_at"=CURRENT_TIMESTAMP WHERE "order_id"=${orderId}
...
export async function markRenewalOrderPaymentFailed(orderId: string, message = "Renewal payment failed."): Promise<void> {
  const rows = await prisma.$queryRaw<{ subscription_id: string }[]>`SELECT "subscription_id" FROM "billing_renewal_attempts" WHERE "order_id"=${orderId} LIMIT 1`;
  const subscriptionId = rows[0]?.subscription_id;
  if (!subscriptionId) return;
  await prisma.$executeRaw`
    UPDATE "billing_renewal_attempts" SET
      "status"='FAILED',"attempted_at"=CURRENT_TIMESTAMP,"failure_code"='PAYMENT_FAILED',
      "failure_message"=${message.slice(0, 500)},"updated_at"=CURRENT_TIMESTAMP
    WHERE "order_id"=${orderId}
  `;
  await prisma.$executeRaw`
    UPDATE "billing_subscriptions" SET
      "status"='PAST_DUE',"failed_payment_count"="failed_payment_count"+1,
      "grace_until"=COALESCE("grace_until",CURRENT_TIMESTAMP+INTERVAL '7 days'),
      "next_billing_at"=CURRENT_TIMESTAMP+INTERVAL '1 day',"updated_at"=CURRENT_TIMESTAMP
