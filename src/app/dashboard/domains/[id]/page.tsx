...
interface DomainDetail {
  ...
  providerName: string;
  recentOrders: Array<{ id: string; description: string; totalCents: number; provisioningStatus: string; createdAt: string; order: { orderNumber: string; status: string; createdAt: string } }>;
...
function Overview({ domain }: { domain: DomainDetail }) {
...
function NameserversTab({ domainId, current, onChange }: { domainId: string; current: string[]; onChange: () => Promise<void> | void }) {
  const [nameservers, setNameservers] = useState<string[]>(current.length ? current : ["", ""]);
  const [error, setError] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);
  useEffect(() => { setNameservers(current.length ? current : ["", ""]); }, [current]);

  async function save() {
    const cleaned = nameservers.map((value) => value.trim().toLowerCase()).filter(Boolean);
