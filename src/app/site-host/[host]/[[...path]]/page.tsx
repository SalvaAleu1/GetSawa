...
async function resolveHost(host: string, path: string[]) {
  ...
  const rows = await prisma.$queryRaw<Array<{ project_id: string; project_status: string }>>`
    ...
    FROM "website_custom_domains" wcd
    JOIN "WebsiteProject" wp ON wp."id"=wcd."project_id"
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const resolved = await resolveHost(host, path);
  if (!resolved?.content) return {};
  const slug = path[0] || "home";
  const page = resolved.content.pages.find((candidate) => candidate.slug === slug) ?? resolved.content.pages.find((candidate) => candidate.slug === "home") ?? resolved.content.pages[0];
...
export default async function CustomHostSite({ params }: Params) {
  ...
  if (!resolved.content) notFound();
  const pageSlug = path[0] || "home";
  const exists = resolved.content.pages.some((page) => page.slug === pageSlug);
  if (!exists && pageSlug !== "home") notFound();
