...
const NAV = [
  ...
  {
    section: "Websites",
    items: [{ href: "/dashboard/websites", label: "My Websites" }],
  },
  {
    section: "Billing",
    items: [
        ...
        <nav className="space-y-6 px-4 pb-8">
          {NAV.map((group) => (
            <div key={group.section}>
              <p className="px-2 text-xs font-semibold uppercase tracking-wide text-ink/40">{group.section}</p>
                ...
                {group.items.map((item) => (
                  <Link
                    ...
                    className={`block rounded-lg px-3 py-2 text-sm font-medium ${
                      pathname === item.href ? "bg-brand-50 text-brand-600" : "text-ink/70 hover:bg-paper"
                    }`}
                  >
                    {item.label}
                  </Link>
          ...
          <button onClick={handleLogout} className="w-full rounded-lg px-3 py-2 text-left text-sm font-medium text-danger hover:bg-danger/5">
            Sign out
          </button>
        </nav>
      </aside>
