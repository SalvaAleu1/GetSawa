...
export default function BillingPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-8 p-6">
    <header>
      ...
      <p className="mt-2 max-w-3xl text-sm text-gray-600">Review domain and hosting renewals, protected invoices, payments and account credits. Renewal prices are refreshed against their authoritative cost and provider rules before payment.</p>
    </header>
    ...
    <section className="space-y-3">
      <div><h2 className="text-xl font-semibold">Recurring services</h2><p className="mt-1 text-sm text-gray-600">Domain and hosting services keep separate provider lifecycles but share one protected billing history.</p></div>
      {subscriptions.length === 0 ? <div className="rounded-xl border p-6 text-sm text-gray-600">No recurring services are currently attached to your account.</div> :
        <div className="overflow-x-auto rounded-xl border"><table className="w-full text-left text-sm"><thead className="border-b bg-gray-50"><tr><th className="p-4">Service</th><th className="p-4">Renewal</th><th className="p-4">Period ends</th><th className="p-4">Invoice check</th><th className="p-4">Automation</th></tr></thead><tbody>
          {subscriptions.map((subscription) => <tr key={subscription.id} className="border-b last:border-0">
            <td className="p-4 font-medium">{subscription.serviceLabel}<div className="text-xs font-normal text-gray-500">{subscription.serviceInstanceId ? "Web hosting" : "Domain"} · {subscription.status}</div></td>
            ...
            <td className="p-4">{subscription.domainId ? <button disabled={busy === subscription.domainId} onClick={() => void toggleDomain(subscription.domainId!, !subscription.autoRenew)} className="rounded-md border px-3 py-1.5 font-medium disabled:opacity-50">{busy === subscription.domainId ? "Saving…" : subscription.autoRenew ? "Enabled" : "Disabled"}</button> : subscription.serviceInstanceId ? <span className="text-xs text-gray-500">Manage from Web Hosting</span> : "Managed by provider"}</td>
          </tr>)}
        </tbody></table></div>}
    </section>
