...
import { recordGrowthConversionForOrder } from "@/lib/growth-attribution";
...
function captureDetails(payload: any) {
  ...
  return { node, completed: payload?.status === "COMPLETED" && node?.status === "COMPLETED", cents: economics.grossCents ?? -1, currency: economics.currency ?? "", providerFeeCents: economics.providerFeeCents };
}

async function reconcilePayPalOrder(providerOrderId: string) { try { return await PayPalProvider.getOrder(providerOrderId); } catch { return null; } }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const containsDomainPricing = order.items.some((item) => item.description.includes(" registration") || item.description.includes(" renewal") || item.description.endsWith(" transfer") || item.description.includes("premium domain purchase"));
    ...
    try { await assertPremiumOrderReservations(order.id, user.id); await assertOrderCreditReservation(order.id, user.id); }
    ...
    try { capture = await PayPalProvider.captureOrder(payment.providerOrderId, `capture-${order.id}`); } catch { capture = await reconcilePayPalOrder(payment.providerOrderId); }
    ...
    if (!details.completed) {
      ...
      if (["VOIDED", "CANCELLED", "DENIED"].includes(status)) {
        ...
        try { await sendEmail({ to: order.user.email, ...emailTemplates.paymentFailed(order.orderNumber) }); } catch {}
    ...
    if (committed) {
      ...
      await Promise.all([
        recordCommissionForOrder(order.id, user.id).catch((err) => console.error("[affiliates] commission recording failed:", err)),
        recordGrowthConversionForOrder(order.id, user.id).catch((err) => console.error("[growth] conversion recording failed:", err)),
      ]);
    }
