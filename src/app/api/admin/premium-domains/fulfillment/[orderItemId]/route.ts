...
export async function POST(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    const item = await prisma.orderItem.findUnique({
      ...
      select: { orderId: true, provisioningStatus: true },
    });

    if (!item) return jsonError("Premium fulfillment item not found.", 404);
    if (item.provisioningStatus !== "MANUAL_REVIEW") {
      return jsonError("This premium item is not awaiting ownership verification.", 409);
    }

    const result = await completePremiumFulfillment({
      orderItemId,
    ...
    if (!result) {
      return jsonError("This premium sale has already been fulfilled.", 409);
    }
