...
function platformHosts(){const hosts=new Set(["localhost","127.0.0.1"]);try{if(process.env.APP_URL)hosts.add(new URL(process.env.APP_URL).hostname.toLowerCase());}catch{}if(process.env.WEBSITE_PLATFORM_HOST)hosts.add(process.env.WEBSITE_PLATFORM_HOST.toLowerCase());return hosts;}
