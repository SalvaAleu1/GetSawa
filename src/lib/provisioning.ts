...
import { provisionCatalogProduct } from "@/lib/product-provisioning";
...
export async function provisionOrder(orderId: string): Promise<void> {
  ...
  await notifyOrderLifecycle({
    orderId: order.id, orderNumber: order.orderNumber, userId: order.userId, email: order.user.email, type: "ORDER_PROVISIONING",
    title: `Order ${order.orderNumber} is being processed`, body: `Your order ${order.orderNumber} has been paid and fulfilment is now in progress.`,
    emailSubject: `Order ${order.orderNumber} is being processed`, emailHtml: `<p>Your order <strong>${escapeHtml(order.orderNumber)}</strong> has been paid and fulfilment is now in progress.</p>`,
  });
  ...
  for (const item of order.items) {
    ...
    try {
      ...
      if (premiumLink) {
        ...
        await provisionCatalogProduct({ order, item });
      } else if (!item.domainId && !item.domainTransferId && item.years) {
        await provisionDomainRegistration(order, item);
      } else if (item.domainId && item.years) {
      ...
      await prisma.orderItem.updateMany({ where: { id: item.id, provisioningStatus: "PROVISIONING" }, data: { provisioningStatus: "FAILED", provisioningNote: `Provisioning failed. Retry is available from the order workflow. Ref: ${item.id}` } });
      try { await logAudit({ actorId: null, action: "ORDER_ITEM_PROVISIONING_FAILED", resource: "order_item", resourceId: item.id, metadata: { orderId: order.id, reason: internalMessage.slice(0, 500) } }); } catch { /* durable FAILED state is authoritative */ }
    }
  }
  ...
  if (allProvisioned) {
    ...
    await notifyOrderLifecycle({ orderId: order.id, orderNumber: order.orderNumber, userId: order.userId, email: order.user.email, type: "ORDER_ACTIVE", title: `Order ${order.orderNumber} is complete`, body: `All services in order ${order.orderNumber} have been successfully provisioned.`, emailSubject: `Order ${order.orderNumber} is complete`, emailHtml: `<p>All services in order <strong>${escapeHtml(order.orderNumber)}</strong> have been successfully provisioned and are now active.</p><p>Total paid: ${escapeHtml(formatCents(order.totalCents, order.currency))}.</p>` });
    ...
    await notifyOrderLifecycle({ orderId: order.id, orderNumber: order.orderNumber, userId: order.userId, email: order.user.email, type: "ORDER_FULFILMENT_FAILED", title: `Action required for order ${order.orderNumber}`, body: `One or more services in order ${order.orderNumber} could not be provisioned automatically. Your payment remains recorded and the failed fulfilment can be retried by support.`, emailSubject: `Action required for order ${order.orderNumber}`, emailHtml: `<p>One or more services in order <strong>${escapeHtml(order.orderNumber)}</strong> could not be provisioned automatically.</p><p>Your payment remains recorded. Our fulfilment workflow can retry the failed service.</p>` });
  }
}

async function claimProvisioningItem(item: OrderItem): Promise<boolean> {
  if (item.provisioningStatus === "PROVISIONING") {
    const match = (item.provisioningNote || "").match(/^CLAIMED:(.+)$/);
    const claimedAt = match ? Date.parse(match[1] ?? "") : NaN;
    if (Number.isFinite(claimedAt) && Date.now() - claimedAt < STALE_CLAIM_MS) return false;
  }
  const result = await prisma.orderItem.updateMany({ where: { id: item.id, provisioningStatus: item.provisioningStatus === "PROVISIONING" ? "PROVISIONING" : { in: ["PENDING", "FAILED"] } }, data: { provisioningStatus: "PROVISIONING", provisioningNote: `CLAIMED:${new Date().toISOString()}` } });
...
async function provisionDomainRegistration(order: Order, item: OrderItem) {
  ...
  if (!availability?.available) { const reconciled = await reconcileRegisteredDomain(provider, domainName); if (!reconciled) throw new Error("The domain is no longer available for registration."); }
  const user = await prisma.user.findUniqueOrThrow({ where: { id: order.userId } });
  const phone = user.phone?.trim() || ""; const country = user.country?.trim().toUpperCase() || "";
  if (!phone || !country) throw new Error("Complete domain contact information is required before registration.");
  let result;
  try { result = await provider.registerDomain({ domain: domainName, years: item.years ?? 1, registrant: { firstName: user.firstName, lastName: user.lastName, email: user.email, phone, address1: "", city: "", zip: "", country }, idempotencyKey: item.id }); }
  catch (error) { const reconciled = await reconcileRegisteredDomain(provider, domainName); if (!reconciled) throw error; result = { success: true, domain: domainName, expiresAt: reconciled.expiresAt }; }
  if (!result.success) { const reconciled = await reconcileRegisteredDomain(provider, domainName); if (!reconciled) throw new Error(result.errorMessage || "Domain registration failed at the registrar."); result = { ...result, success: true, expiresAt: reconciled.expiresAt }; }
  const domain = await prisma.domain.upsert({ where: { name: domainName }, create: { userId: order.userId, tldId: tldRecord.id, name: domainName, status: "ACTIVE", isPremium: Boolean(availability?.isPremium), providerName: provider.name, providerOrderId: result.providerOrderId, registeredAt: new Date(), expiresAt: result.expiresAt ? new Date(result.expiresAt) : undefined, autoRenew: true, isLocked: true }, update: { status: "ACTIVE", isPremium: Boolean(availability?.isPremium), registeredAt: new Date(), expiresAt: result.expiresAt ? new Date(result.expiresAt) : undefined, providerName: provider.name, providerOrderId: result.providerOrderId } });
...
async function reconcileRegisteredDomain(provider: ReturnType<typeof getDomainProvider>, domainName: string) { try { const info = await provider.getDomainInfo(domainName); return info?.domain && info.expiresAt ? info : null; } catch { return null; } }
...
async function provisionDomainTransfer(order: Order, item: OrderItem) {
  ...
  const provider = getDomainProvider(); const authCode = decryptSecret(transfer.authCodeEncrypted);
...
function escapeHtml(input: string): string { return input.replace(/[&<>\"']/g, (character) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '\"': "&quot;", "'": "&#39;" }[character] as string)); }
