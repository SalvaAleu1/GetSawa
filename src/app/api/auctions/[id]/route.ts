...
export async function GET(_req: NextRequest, { params }: RouteContext) {
  try {
    ...
    const bids = await prisma.auctionBid.findMany({ where: { auctionId: id }, orderBy: { amountCents: "desc" }, take: 20, include: { user: { select: { firstName: true, lastName: true } } } });
    const user = await getCurrentUser();
    const isWinner = Boolean(user && auction?.winningBidId && bids.find((b) => b.id === auction!.winningBidId)?.userId === user.id);
    return jsonOk({ auction, isWinner, bids: bids.map((b, i) => ({ id: b.id, amountCents: b.amountCents, createdAt: b.createdAt, bidder: i === 0 ? "Highest bidder" : `Bidder #${bids.length - i}` })) });
