...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (order.status !== "PENDING_PAYMENT") {
      return jsonOk({ status: order.status });
    }

    const payment = order.payments.find((p) => p.status === "PENDING");
    if (!payment?.providerOrderId) return jsonError("No pending payment found.", 400);

    const capture = await PayPalProvider.captureOrder(payment.providerOrderId, `capture-auction-${order.id}`);
    const captureNode = capture?.purchase_units?.[0]?.payments?.captures?.[0];

    if (capture.status !== "COMPLETED" || captureNode?.status !== "COMPLETED") {
    ...
    // Attempt registration through NameSilo — this assumes the auctioned
    // domain is available to register fresh. If GetSawa auctions domains it
    // does not directly control at the registry, this step should instead
    // trigger a manual/admin handoff; that path is not built in this phase.
    let provisioningNote: string | null = null;
    try {
      const ext = auction.domainName.split(".").slice(1).join(".");
      ...
      if (tld && provider.isConfigured()) {
        const result = await provider.registerDomain({
          ...
          registrant: {
            ...
            city: "",
            zip: "",
            country: order.user.country || "US",
          },
          idempotencyKey: `auction-${auction.id}`,
        });
        if (result.success) {
          await prisma.domain.upsert({
            where: { name: auction.domainName },
            create: {
              userId: order.userId,
              tldId: tld.id,
              name: auction.domainName,
              status: "ACTIVE",
              isPremium: true,
            ...
            update: { userId: order.userId, status: "ACTIVE" },
          });
          await prisma.order.update({ where: { id: order.id }, data: { status: "ACTIVE" } });
        } else {
          provisioningNote = result.errorMessage || "Registration failed.";
        }
      } else {
        provisioningNote = "Domain provider not configured — an administrator needs to complete this transfer manually.";
      }
    } catch (err: any) {
      provisioningNote = err.message;
    }

    if (provisioningNote) {
      await prisma.order.update({ where: { id: order.id }, data: { status: "PROVISIONING", provisioningError: provisioningNote } });
    }
